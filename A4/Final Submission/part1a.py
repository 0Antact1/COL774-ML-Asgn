...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dir == 'SyntheticData':
            image = Image.open(
                f"{self.pdir}/col_774_A4_2023/{self.dir}/images/{img_name}")
        else:
            image = Image.open(
                f"{self.pdir}/col_774_A4_2023/{self.dir}/images/train/{img_name}")

        if self.transform:
            image_tensor = self.transform(image)
            if self.dir != 'SyntheticData':
                image_tensor = torch.cat(
                    (image_tensor, image_tensor, image_tensor), dim=0)
            ...
            for _ in range(max_len):
                ...
                if _ == 0:
                    ...
                    predicted_tokens = torch.cat(
                        (predicted_tokens, predicted_token), dim=1)
...
def load_and_predict(dirn='.'):
    ...
    val_hw_csv = pd.read_csv(f'{dirn}/col_774_A4_2023/HandwrittenData/val_hw.csv')

    sus_data = TestDataset(f"{dirn}/col_774_A4_2023/HandwrittenData/val_hw.csv", pdir=dirn, directory='HandwrittenData')
    ...
    val_pred_df.to_csv('pred1a', index=False)
